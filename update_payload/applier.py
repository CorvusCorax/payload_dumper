...
def _ReadExtents(file_obj, extents, block_size, max_length=-1):
  ...
  for ex in extents:
    ...
    elif (type(file_obj) is bool):
      data.extend(itertools.repeat('\xef', read_length))
    else:
...
class PayloadApplier(object):
  ...
  def _ApplyDiffOperation(self, op, op_name, patch_data, old_part_file,
    ...
    if (hasattr(new_part_file, 'fileno') and (not (type(old_part_file) is bool)) and
      ...
      input_part_file = old_part_file if (old_part_file or (type(old_part_file) is bool)) else new_part_file
      ...
      with open(out_file_name, 'rb') as out_file:
        ...
        if len(out_data) != data_length:
          raise PayloadError(
              '%s: actual patched data length (%d) not as expected (%d)' %
              (op_name, len(out_data), data_length))
